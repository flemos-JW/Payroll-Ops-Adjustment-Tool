...
# Layout
# ---------------------------------------------------------------------------
tab_calc, tab_fica, tab_ficad, tab_upload = st.tabs(["Calculator", "FICA Refund", "FICA Debit", "Data Upload"])
...
with tab_upload:
    ...
    st.caption("Upload any file here to share data. It will be saved to disk so it can be used to update the calculator.")
    ...
    if uploaded_file is not None:
        ...
        try:
            ...
            if df is not None:
                ...
                save_path = os.path.join(os.path.dirname(__file__), "uploaded_data.csv")
                df.to_csv(save_path, index=False)

            # Always save raw bytes too
            raw_path = os.path.join(os.path.dirname(__file__), "uploaded_raw_" + uploaded_file.name)
            with open(raw_path, "wb") as f:
                f.write(file_bytes)

            st.success(f"Saved **{uploaded_file.name}**" + (f" — {len(df):,} rows × {len(df.columns)} columns" if df is not None else ""))
        except Exception as e:
            st.error(f"Could not read file: {e}")

    if "uploaded_df" in st.session_state:
        st.markdown(f"**Active file:** {st.session_state.get('uploaded_name', 'unknown')}")
